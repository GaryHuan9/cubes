...
__global__
void new_path(UInt2 resolution, Camera* camera,  List<NewPathPackets> packets, List<TracePackets> results);

__global__
void trace_rays(List<TracePackets> packets, Array<HitPacket> results);

__global__
void render_end(UInt2 resolution, Array<TracePackets> packets, Array<HitPacket> results, Array<Accumulator> accumulators);
